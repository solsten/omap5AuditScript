...
#public functions
    ...
    def getContinuityTests(self):
        if not self.__dataPulled:
            self.__pullData()
        return self.__opensShortsTests
        ...
        with open(self.__datalogFileName,"r") as datalogFile:
            for (lineNum,line) in enumerate(datalogFile):
                if lineNum in self.__initSeqLineNums:
                    ...
                    if testInst not in self.__testInstancesFound:
                        #if self.__isOpensShortsTest(line):
                            #self.__storeOpensShortsTestInstance(testInst, lineNum)
                        #else:
    ...
    def __isOpensShortsTest(self,line):
        isOpensShorts = True if "SUPPLYSHORTS" in line else False
        return isOpensShorts
    ...
    def __storeOpensShortsTestInstance(self,testInst,lineNum):
        self.__testInstancesFound.append(testInst)
        testInst = tuple([lineNum]) + testInst
        self.__testInstances.append(testInst)
        self.__opensShortsTests.add(testInst[MEAS_NAME])
